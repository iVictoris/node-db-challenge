...
-   [x] Explain the difference between `Relational Databases` and `SQL`. SQL is a query language, and relational databases are just databases with tables that have relationships with other tables.

-   [x] Why do tables need a `primary key`? to ID a row

-   [x] What is the name given to a table column that references the primary key on another table. foreign key

-   [x] What do we need in order to have a _many to many_ relationship between two tables. another table that uses a composite key composed of the primary key of 2 or more tables.
...
-   [x] Bild an API with endpoints for:
    -   [x] adding resources.
